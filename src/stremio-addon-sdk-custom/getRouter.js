...
function getRouter({ manifest, get }) {
    ...
    function manifestHandler(req, res) {
        ...
        if (
            ...
            manifestClone.catalogs = manifestClone.catalogs.filter(
                (catalog) => parsedConfig[catalog.id] === 'on'
            )
                    ...
                    const cacheControl = Object.keys(cacheHeaders)
                        .map((prop) => {
                            ...
                            if (cacheValue > 365 * 24 * 60 * 60)
                                console.warn(
                                    `${prop} set to more then 1 year, be advised that cache times are in seconds, not milliseconds.`
                                )
